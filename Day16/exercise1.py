...
def getIntValueOfBinary(input: str, length: int) -> int:
...
def resolve(input: str) -> tuple[int, str]:
    ...
    if T == 4: # literal operator
        while input[:1] == '1':
            input = input[5:] # ignore literal value
        return (V, input[5:]) # strip last literal value
    else: # 
        I, input = getIntValueOfBinary(input, 1)
        returnValue = V
        if I == 0: # 15 bits total length in bits
            L, input = getIntValueOfBinary(input, 15)
            while input != "" and int(input) != 0: # skip last 0 bits
...
sumOfVersions = resolve(binary_input)[0]

print(f"Done! Answer is '{sumOfVersions}' for question Decode the structure of your hexadecimal-encoded BITS transmission; what do you get if you add up the version numbers in all packets?, --- {(time.time() - start_time)} seconds ---")

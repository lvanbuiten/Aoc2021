...
values_s = input
values_s_len = len(values_s)
print(f'Template\t: {values_s}')
for step in range(1, 11):
    tempValues = list(values_s[:])
    for i, value in enumerate(tempValues):
        ...
        resolved_val = pairs[value + tempValues[i+1]]
        values_s = values_s[:next_index] + resolved_val + values_s[next_index:]

    values_s_len = len(values_s)
    print(f'After step {step}\t: {values_s}')
